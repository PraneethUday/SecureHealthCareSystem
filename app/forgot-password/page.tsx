...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log(
        "[Forgot Password Frontend] Response status:",
        response.status,
      );
      ...
      if (
        errorDetails.includes("Unexpected token") ||
        errorDetails.includes("non-JSON")
      ) {
        setError(
          "Server error (500): The password reset service is currently failing. Check your server terminal logs.",
        );
      } else {
