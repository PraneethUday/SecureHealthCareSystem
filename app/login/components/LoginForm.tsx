  ...
  return (
    <form onSubmit={onSubmit} className={isPatient ? "space-y-3" : "space-y-5"}>
      ...
      <div>
        <label
          htmlFor="identifier"
          className={`block ${isPatient ? "text-xs" : "text-sm"
            } font-medium text-gray-700 dark:text-gray-300 ${isPatient ? "mb-1" : "mb-2"}`}
        >
          {inputLabel}
        </label>
        <div className="relative">
          <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            <InputIcon
              className={`${isPatient ? "w-4 h-4" : "w-5 h-5"} ${themeClasses.icon
          ...
          <input
            ...
            className={`w-full ${isPatient ? "pl-9 pr-3 py-2" : "pl-10 pr-4 py-3"
      ...
      <div className="flex justify-end">
        <a
          ...
          className={`text-xs ${themeClasses.link} font-medium transition-colors duration-200 hover:underline`}
        >
          Forgot password?
        </a>
      </div>
