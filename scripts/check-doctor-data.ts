...
async function checkDoctorData() {
  ...
  doctors.forEach((doc) => {
    ...
    console.log(
      `  Password field: ${doc.password ? `"${doc.password}"` : "NULL"}`,
    );
    console.log(`  Password length: ${doc.password ? doc.password.length : 0}`);
    console.log(
      `  Password hash: ${doc.password_hash ? "EXISTS (hashed)" : "NULL"}`,
    );
  ...
  doctors.forEach((doc) => {
    ...
    console.log(
      `${doc.doctor_id}: "${testPwd}" === "${doc.password}" ? ${matches ? "✅ MATCH" : "❌ NO MATCH"}`,
    );
  });
}
