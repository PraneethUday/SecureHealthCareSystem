...
if (!supabaseUrl || !supabaseKey) {
  ...
  console.error('   SUPABASE_SERVICE_ROLE_KEY or ANON_KEY:', supabaseKey ? '✅ Set' : '❌ Missing');
  process.exit(1);
}
...
async function fixDoctorLogins() {
  ...
  const doctors = [
    { id: 'D001', password: 'doctor1', name: 'Dr. Rajesh Kumar', specialty: 'Cardiology' },
    { id: 'D002', password: 'doctor2', name: 'Dr. Priya Selvam', specialty: 'Pediatrics' },
    { id: 'D003', password: 'doctor3', name: 'Dr. Lakshmi Narayanan', specialty: 'Neurology' },
  ];
  ...
  const { data: allDoctors, error: fetchError } = await supabaseAdmin
    .from('doctors')
    .select('doctor_id, first_name, last_name, specialization, is_locked, login_attempts, is_mfa_enabled')
