...
async function testAppointmentsSetup() {
  ...
  hospitals.forEach(h => console.log(`   - ${h.name} (${h.city}, ${h.state})`));
  console.log('');
  ...
  console.log(`✅ Appointments table exists (${appointments?.length || 0} appointments)`);
  console.log('');
  ...
  if (!doctors || doctors.length === 0) {
    ...
    doctors.forEach(d => console.log(`   - Dr. ${d.first_name} ${d.last_name} (${d.specialization})`));
  }
  console.log('');
  ...
  if (!patients || patients.length === 0) {
    ...
    patients.forEach(p => console.log(`   - ${p.first_name} ${p.last_name} (${p.patient_id})`));
  }
  console.log('');
...
// Run the test
testAppointmentsSetup().then(success => {
  if (success) {
    console.log('✅ ============================================');
