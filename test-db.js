  ...
  try {
    ...
    if (patientsError) {
      ...
      if (patientsError.message.includes('blood_group')) {
        ...
        console.log('ALTER TABLE patients ADD COLUMN IF NOT EXISTS phone_number TEXT;');
        console.log('ALTER TABLE patients ADD COLUMN IF NOT EXISTS gender TEXT;');
        console.log('ALTER TABLE patients ADD COLUMN IF NOT EXISTS emergency_contact TEXT;');
        console.log('ALTER TABLE patients ADD COLUMN IF NOT EXISTS blood_group TEXT;\n');
      }
      return;
    }
    ...
    if (patients && patients.length > 0) {
      ...
      const requiredColumns = ['phone_number', 'gender', 'emergency_contact', 'blood_group'];
    ...
    if (logsError) {
      ...
      console.log(`✅ Access logs working (${logs?.length || 0} recent entries)`);
      if (logs && logs.length > 0) {
        console.log('   Recent activity:');
        logs.slice(0, 3).forEach(log => {
          console.log(`   - ${log.action} by ${log.user_role} (${log.user_id})`);
        });
      }
    }
    console.log('');
